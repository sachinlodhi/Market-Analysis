...
options =[[
        ...
        [psg.Text("Verbosity", key="verbosity"),
              ...
              [psg.Output(size=(85, 10), background_color='white', text_color='black', key='Status_Box',
                          visible=False)],
...
def gui():
    ...
    layout = [
        [psg.Column(options),psg.VerticalSeparator(),psg.Column(graph, element_justification='t')]
    ]
    ...
    while True:
        ...
        if event == 'Plot':
            ...
            fig = plt.gcf()
            DPI = fig.get_dpi()
            # Adjust this size to reduce the movement error when the mouse hovers over the figure, it's close to canvas size
            ...
            plt.plot(graph_data)
            filename = str(values['ticker'] + "_" + values['s_Date'] + ".png")
            plt.savefig(filename)
            print(f"[INFO] Graph saved : {filename}")
